...
export default function Headcount() {
  ...
  const handleFormChange = (field, value) => {
    ...
    // Auto-generate uniqueKey when month or totals change
    if (field === 'month' || field.includes('total') || field === 'date') {
      const month = field === 'month' ? value : newFormData.month;
      const totalSgd = newFormData.total_sgd || newFormData['total sgd'];
      const totalMyr = newFormData.total_myr || newFormData['total myr'];
      const totalUsc = newFormData.total_usc || newFormData['total usc'];
      
      if (month) {
        const uniqueKey = generateUniqueKey(month, totalSgd, totalMyr, totalUsc);
  ...
  const handleEditChange = (field, value) => {
    ...
    // Auto-generate uniqueKey when date or totals change in edit mode
    if (field === 'date' || field.includes('total') || field.includes('css_') || field.includes('sr_') || field.includes('cashier_')) {
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const generatedUniqueKey = generateUniqueKey(month, totalSgd, totalMyr, totalUsc);
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          <span>ℹ️</span>
                          <span>YEAR, MONTH, dan UNIQUEKEY akan otomatis terisi berdasarkan DATE dan TOTAL yang Anda input</span>
                        </div>
                      </div>
                      <div style={{
