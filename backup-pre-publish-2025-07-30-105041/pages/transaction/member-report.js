...
export default function MemberReport() {
  ...
  // DATA STATES
  const [member_reportData, setMember_reportData] = useState([]);
  ...
  const fetchMember_reportData = async () => {
    try {
      setLoading(true);
      const params = new URLSearchParams({
        currency,
      ...
      if (result.success) {
        setMember_reportData(result.data);
        setPagination(result.pagination);
      } else {
        console.error('Error fetching data:', result.error);
        setMember_reportData([]);
      }
    } catch (error) {
      console.error('Error fetching member_report data:', error);
      setMember_reportData([]);
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const filename = contentDisposition 
          ...
          : 'member_report_daily_export.xlsx';
        ...
        <div style={{
          ...
          <div style={{ 
            ...
            <button
              ...
              disabled={isReadOnly || !canExportData || exporting || member_reportData.length === 0}
              style={{
                padding: '8px 16px',
                borderRadius: '6px',
                border: 'none',
                fontSize: '14px',
                fontWeight: '600',
                backgroundColor: isReadOnly || !canExportData || member_reportData.length === 0 ? '#f3f4f6' : '#10b981',
                color: isReadOnly || !canExportData || member_reportData.length === 0 ? '#9ca3af' : 'white',
                cursor: isReadOnly || !canExportData || member_reportData.length === 0 ? 'not-allowed' : 'pointer',
        ...
        <div style={{ 
          ...
          {loading ? (
            <div style={{
              ...
              Loading member_report data...
            </div>
          ) : member_reportData.length === 0 ? (
            <div style={{
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
              ...
              <div style={{ fontSize: '18px', color: '#6b7280' }}>
                No member_report data found for the selected filters
              </div>
            </div>
          ) : (
            <div className="data-table-container">
                 ...
                 <p style={{ margin: '8px 0 0 0', color: '#6b7280', fontSize: '14px' }}>
                   Showing {member_reportData.length} of {pagination.totalRecords.toLocaleString()} records
                 </p>
               </div>
               ...
               <div className="table-wrapper">
                 <table className="data-table">
                   <thead>
                     <tr>
                       {member_reportData.length > 0 && Object.keys(member_reportData[0]).map((column, index) => (
                         <th key={index}>{column.replace(/_/g, ' ').toUpperCase()}</th>
                       ))}
                     </tr>
                   </thead>
                   <tbody>
                     {member_reportData.map((row, rowIndex) => (
                       <tr key={rowIndex}>
