...
// List of transaction pages to convert to Coming Soon
...
function createComingSoonPage(filePath) {
  console.log(`🔧 Converting ${filePath} to Coming Soon...`);
  
  const pageName = filePath.split('/').pop().replace('.js', '');
  const title = pageName.charAt(0).toUpperCase() + pageName.slice(1) + ' Transactions';
...
export default function ${pageName.charAt(0).toUpperCase() + pageName.slice(1)}() {
  const { user, loading: authLoading } = useRoleAccess('/transaction/${pageName}');
  ...
  console.log(`✅ Converted ${filePath} to Coming Soon`);
}

// Convert all transaction pages
transactionPages.forEach(createComingSoonPage);

console.log('🎉 All transaction pages converted to Coming Soon!'); 