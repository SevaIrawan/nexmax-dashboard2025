...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('📊 Exporting new_depositor data with filters:', { currency, line, year, month, startDate, endDate, filterMode });

    // Base query
    let query = `SELECT * FROM new_depositor WHERE 1=1`;
    let params = [];
    let paramCount = 0;
    ...
    console.log(`✅ Exporting ${result.rows.length} records from new_depositor`);

    // Create Excel workbook
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('New Depositor Data');
    ...
    const filename = `new_depositor_export_${timestamp}.xlsx`;
    ...
    console.error('❌ Error exporting new_depositor data:', error);
    res.status(500).json({ 
      success: false,
